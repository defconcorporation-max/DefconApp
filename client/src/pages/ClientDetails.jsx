...
const ClientDetails = () => {
    ...
    const handleFlightLookup = () => {
        ...
        const mockFlightData = {
            ...
            start_time: moment().add(1, 'days').set({ hour: 10, minute: 0 }).format('YYYY-MM-DDTHH:mm'),
            end_time: moment().add(1, 'days').set({ hour: 14, minute: 30 }).format('YYYY-MM-DDTHH:mm'),
                            ...
                            {client.wishlist.map((item, index) => (
                                <div key={index} className="bg-dark-800/40 border border-pink-500/20 rounded-xl p-4 flex gap-4 group hover:bg-dark-800/60 transition">
                                    <div className="w-16 h-16 rounded-lg bg-dark-900 overflow-hidden shrink-0">
                                        {item.image_url ? (
                                            <img src={item.image_url.startsWith('http') ? item.image_url : `${API_URL}/${item.image_url.replace(/^\//, '')}`} alt={item.title} className="w-full h-full object-cover" />
                                        ) : (
                                                ...
                                                <div className="flex gap-4">
                                                    ...
                                                    <div className="flex-1">
                                                        ...
                                                        {item.image_url && (
                                                            <div className="mt-4">
                                                                <img
                                                                    src={item.image_url.startsWith('http') ? item.image_url : `${API_URL}/${item.image_url.replace(/^\//, '')}`}
                                                                    alt="Attachment"
