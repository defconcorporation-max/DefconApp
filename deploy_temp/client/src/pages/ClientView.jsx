        ...
        <div className="bg-white dark:bg-dark-800/60 border border-slate-200 dark:border-white/5 rounded-2xl p-5 mb-4 shadow-sm dark:shadow-none relative overflow-hidden group">
            ...
            <div className="flex gap-4">
                <div className="w-24 h-24 rounded-xl bg-slate-200 dark:bg-white/5 overflow-hidden flex-shrink-0">
                    {hotel.image_url ? (
                        <img src={hotel.image_url} alt={hotel.title} className="w-full h-full object-cover" />
                    ) : (
                                ...
                                <div key={item.id} className="relative pl-12 group">
                                    ...
                                    >
                                        ...
                                        <div className="relative z-10">
                                            ...
                                            {item.description && (
                                                <p className="text-slate-600 dark:text-slate-500 text-sm line-clamp-2 mb-3">{item.description}</p>
                                            )}
                                        </div>
                                    </div>
                                </div>
            ...
            <div className="min-h-screen bg-gray-50 dark:bg-dark-950 text-slate-900 dark:text-slate-200 font-sans selection:bg-primary-500/30 transition-colors duration-300">
                ...
                /* <div className="mb-8 text-center">
                    <h1 className="text-3xl md:text-4xl font-bold text-white mb-2">
                        {t('clientView.welcome', { name: client.name.split(' ')[0] })}
                    </h1>
                    <p className="text-slate-400">
                        {t('clientView.tripDates', { start: moment(client.trip_start).format('MMMM D'), end: moment(client.trip_end).format('MMMM D, YYYY') })}
                    </p>
                </div> */}
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        >
                            ...
                            {activeTab === 'flights' && (
                                <div className="space-y-8">
                                    <FlightBoard flights={itinerary.filter(i => i.type === 'flight')} />
                                    <ItineraryList items={itinerary.filter(i => i.type === 'flight')} />
                                </div>
                            )}

                            {activeTab === 'hotels' && (
                                <div className="space-y-8">
                                    <ItineraryList items={itinerary.filter(i => i.type === 'hotel')} />
                                </div>
                            )}

                            {activeTab === 'calendar' && (
                                <div className="h-[600px] bg-white dark:bg-dark-800/50 rounded-2xl p-4 border border-slate-200 dark:border-white/5 shadow-sm dark:shadow-none">
                                    <Calendar
                                        localizer={localizer}
                    ...
                    {selectedPass && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm" onClick={() => setSelectedPass(null)}>
                            ...
                            >
                                ...
                                <img src={selectedPass.image_url} alt={selectedPass.title} className="w-full h-auto max-h-[80vh] object-contain bg-black" />
                    ...
                    {selectedEvent && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={() => setSelectedEvent(null)}>
                            ...
                            >
                                ...
                                <div className={`h-32 w-full relative ${selectedEvent.type === 'flight' ? 'bg-blue-600' :
                                    selectedEvent.type === 'hotel' ? 'bg-purple-600' :
                                        'bg-primary-600'
                                    }`}>
                                    {selectedEvent.image_url && (
                                        <>
                                            <img src={selectedEvent.image_url} alt={selectedEvent.title} className="w-full h-full object-cover opacity-50" />
                                            <div className="absolute inset-0 bg-gradient-to-t from-dark-900 to-transparent"></div>
                                        </>
                                    )}
                                    <button
                                        onClick={() => setSelectedEvent(null)}
                                        className="absolute top-4 right-4 p-2 bg-black/30 text-white rounded-full hover:bg-black/50 transition backdrop-blur-md"
                                    >
                                        <X size={20} />
                                    </button>
                                    <div className="absolute bottom-4 left-6">
                                        <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold uppercase tracking-wider mb-2 ${selectedEvent.type === 'flight' ? 'bg-blue-500/20 text-blue-100 border border-blue-500/30' :
                                            selectedEvent.type === 'hotel' ? 'bg-purple-500/20 text-purple-100 border border-purple-500/30' :
                                                'bg-primary-500/20 text-primary-100 border border-primary-500/30'
                                            }`}>
                                            {selectedEvent.type === 'flight' ? <Plane size={12} /> :
                                                selectedEvent.type === 'hotel' ? <Hotel size={12} /> :
                                                    <Ticket size={12} />}
                                            {selectedEvent.type || 'Activity'}
                                        </div>
                                        <h2 className="text-2xl font-bold text-white shadow-sm">{selectedEvent.title}</h2>
                                    </div>
                                </div>
                                ...
                                <div className="p-6 space-y-6">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="bg-slate-50 dark:bg-dark-800/50 p-3 rounded-xl border border-slate-200 dark:border-white/5">
                                            ...
                                            <div className="text-slate-900 dark:text-white font-medium">
                                                {selectedEvent.start_time && format(parseISO(selectedEvent.start_time), 'MMM d, HH:mm')}
                                            </div>
                                            {selectedEvent.end_time && (
                                                <div className="text-slate-500 text-sm">
                                                    to {format(parseISO(selectedEvent.end_time), 'HH:mm')}
                                                </div>
                                            )}
                                        </div>
                                        {selectedEvent.location && (
                                            <div className="bg-slate-50 dark:bg-dark-800/50 p-3 rounded-xl border border-slate-200 dark:border-white/5">
                                                ...
                                                <AddressLink location={selectedEvent.location} className="text-slate-900 dark:text-white font-medium truncate" />
                                            </div>
                                        )}
                                    </div>
                                            ...
                                            >
                                                <div className="flex items-center gap-3">
                                                    <div className="w-10 h-10 rounded-lg bg-slate-200 dark:bg-dark-900 overflow-hidden">
                                                        <img src={selectedEvent.image_url} className="w-full h-full object-cover" alt="Attachment" />
                                                    </div>
