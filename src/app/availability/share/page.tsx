 ...
 * Clients can navigate month-by-month to see available/booked slots.
 */
export default async function PublicAvailabilityPage() {
    const { slots, shoots } = await getAvailabilitySlots();

    // Build a busy map for the client component
    ...
    for (const slot of slots) {
        if (slot.is_booked) {
            ...
            busyBlocks.push({ date: dateKey, startHour: start.getHours(), endHour: end.getHours() });
        }
    }
